...
export function runEntrypoint<TConfig, TSecrets>(
	...
	Promise.resolve()
		.then(async () => {
			...
			// if (!pkgJson || pkgJson.name !== '@companion-surface/base')
			// 	throw new Error('Failed to find the package.json for @companion-surface/base')
			// if (!pkgJson.version) throw new Error('Missing version field in the package.json for @companion-surface/base')
